...
工具分组（可按 include 选择）:
  ...
  note             NoteTool（长时程笔记）
  rag              RagTool（RAG 全流程）
  llamaindex       LlamaIndexRAGTool（LlamaIndex 混合检索 RAG）
  structured_data  StructuredDataTool（自然语言 → SQL 查询）
  vl_media         VLMediaTool（多模态图像/视频/混合分析）
...
def _register_basic(registry: ToolRegistry):
    from src.tools.framework.tool_system import CalculatorTool, TimeTool, SearchTool
...
_TOOL_GROUPS: Dict[str, Callable] = {
    ...
    "structured_data": _register_structured_data,
    "vl_media": _register_vl_media,
...
def build_role_tool_filter() -> ToolFilter:
    ...
    tf.set_policy("writer", allow=[
        ...
        "LlamaIndexRAGTool", "StructuredDataTool", "VLMediaTool",
    ])
