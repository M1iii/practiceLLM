...
from core.cache import SafeFullCache   # 复用缓存层
...
class NoteTool(Tool):
    ...
    def _evict_search_by_note(self, note_id: str):
        ...
        for key in list(self._search_cache.cache.keys()):
            value = self._search_cache.cache.get(key)
...
if __name__ == "__main__":
    ...
    tool.execute("read_note", note_id=first_id)   # 二次读取命中缓存
    ...
    # 6b. 缓存命中验证（删除前）
    print("\n--- 6b) 缓存命中验证（删除前）---")
    tool.execute("list_notes", limit=10)          # 二次列表命中缓存
    tool.execute("summary", recent=3)             # 二次摘要命中缓存
    ...
    print(f"   删除后缓存: 读取 size={tool._note_cache.stats()['size']} | "
          ...
          f"列表 size={tool._list_cache.stats()['size']} | "
