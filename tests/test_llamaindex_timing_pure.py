...
用法：
  cd 项目根目录
  python tests/test_llamaindex_timing_pure.py
"""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"对比分析（与启用 MQE+HyDE 对比）:")
    print(f"{'='*60}")
    print(f"  纯混合检索总耗时: {t_total:.2f}s")
    print(f"  MQE+HyDE 总耗时参考: ~8.5s")
    print(f"  --------------------------")
    print(f"  LLM 调用次数: 1 次（仅问答合成）")
    print(f"  检索耗时: {t_dense+t_sparse+t_fusion:.4f}s")
    print(f"  问答合成耗时: {t_synth:.4f}s")
    print(f"  {'='*60}")
    print(f"结论: 禁用 MQE 和 HyDE 后，响应时间从 ~8.5s 降至 ~{t_total:.1f}s")
    print(f"      约节省 {100 - t_total/8.5*100:.0f}% 的时间，代价是减少召回覆盖")
